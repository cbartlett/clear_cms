class ClearCMS::UsersController < ApplicationController
  ...
  def create
    ...
    @clear_cms_user = ClearCMS::User.new(params[:clear_cms_user])
  ...
  def update
    ...
    respond_to do |format|
      if @clear_cms_user.update_attributes(params[:clear_cms_user])
        format.html { redirect_to edit_clear_cms_user_path(@clear_cms_user), notice: 'User was successfully updated.' }
        format.json { head :ok }
      else
